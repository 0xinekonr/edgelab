# Winsock HTTP Client 手写参考稿

本文是 `device_simulator` 第一版 HTTP 客户端的手写参考稿。请在 CLion 中并排打开本文与源码，
由你手动更新 `http_client.h` 和 `http_client.cpp`。

这版代码刻意保留底层实现，用于学习：

- HTTP 请求如何建立在 TCP socket 之上。
- Windows Winsock 的初始化、域名解析、连接、发送、接收和清理流程。
- C++ 如何使用 RAII 管理 C API 资源。
- C++ 的禁止拷贝、移动语义、`operator=`、`this`、`*this`、`constexpr` 等语言特性。
 ...
 */
[[nodiscard]] HttpResponse post_json(const HttpEndpoint& endpoint,
...
SocketHandle connect_to_endpoint(const edgelab::simulator::HttpEndpoint& endpoint) {
    ...
    const int lookup_result =
        getaddrinfo(endpoint.host.c_str(), port_text.c_str(), &hints, &raw_results);
...
Winsock 间接引入的 Windows 头文件定义了 `max(a, b)` 宏。预处理器在 C++ 编译前先处理宏，
因此它看见 `numeric_limits<...>::max()` 时，会优先尝试展开 Windows 的 `max` 宏。

本参考稿做了两层防护：

1. 在 Windows 头文件前定义 `NOMINMAX`，从源头禁止 `min/max` 宏。
2. 使用 `(std::numeric_limits<T>::max)()`，即使某个其他头文件提前定义过宏，也能阻止它匹配。

同时必须显式包含 `<limits>`。这符合 include what you use：使用哪个标准库类型，就直接包含
声明它的头文件，不依赖其他头文件的间接包含。
